...
- the `ggplot()` function binds the plot to a specific data frame using the  
      `data` argument
...
- this code creates an empty graph, but to see the data we must add the next layer by specifying the type of plot. We can specify the type of plot with a geom function. 
...
- Each geom function takes mapping agruments that tells the function how variales in your dataset will be displayed. The mapping argument is always paired with aes(), which is short for aesthetics. aes() allows us to specify or map the x and y arguments in our dataset to the plot as well as other aesthetic arguments we will explore further below. To add a geom to the plot use the `+` operator. Because we have two continuous variables,  let's use `geom_point()` first:

```{r first-ggplot, purl=FALSE}
ggplot(data = surveys_complete) + geom_point(mapping = aes(x = weight, y = hindfoot_length))
```
...
> ```{r, eval = FALSE}
> surveys_plot +
>  geom_hex(mapping = aes(x = weight, y = hindfoot_length))
> ```
>
...
ggplot(data = surveys_complete) +
    geom_point(mapping = aes(x = weight, y = hindfoot_length), alpha=0.1)
```
...
ggplot(data = surveys_complete) +
    geom_point(mapping = aes(x = weight, y = hindfoot_length), alpha=0.1, color="blue")
```
...
ggplot(data = surveys_complete) +
    geom_point(mapping = aes(x = weight, y = hindfoot_length, color=species_id), alpha = 0.1)
```
...
We can use the same template we introduced in the beginning of the lesson and use boxplots to visualize the distribution of weight within each species:
...
ggplot(data = surveys_complete) +
    geom_boxplot(mapping= aes(x = species_id, y = weight), alpha=0) +
...
ggplot(data = yearly_counts) +
    geom_line(mapping= aes(x = year, y = n, group = species_id))
```
...
ggplot(data = yearly_counts) +
    geom_line(mapping= aes(x = year, y = n, color = species_id))
```
...
ggplot(data = yearly_weight) +
    geom_line(mapping= aes(x=year, y=avg_weight)) +
    facet_wrap(~ species_id) +
    theme_bw()
```
-->
...
ggplot(data = yearly_sex_weight) +
    geom_line(mapping= aes(x = year, y = avg_weight, color = species_id)) +
    facet_grid(sex ~ .)
```
